...
- 현재 구현은 `search` 단일 명령과 `--mode direct|explore`를 제공하는 Search-First `CLI`다
...
- `search` 단일 명령만 구현되어 있고 `--mode direct|explore`로 정렬 정책을 바꿀 수 있다
...
- ranking은 in-memory `tantivy`의 multi-field `BM25`를 사용한다
...
- `Direct matches`: exact symbol match를 우선 배치한 결과
- `Related matches`: direct hit 주변의 관련 결과

## 구현 요약

- `tree-sitter`로 function, method, type, local binding target을 추출한다
- 파일별 분석은 `rayon` `par_iter()`로 병렬 실행한다
- 각 target에서 `symbol`, `signature`, `context` field를 만든다
- in-memory `tantivy` 인덱스에 field boost를 적용해 `BM25`로 점수화한다
- `direct` 모드에서는 exact `Function`/`Method`/`Type`를 먼저, 없을 때만 exact `LocalBinding`을 앞에 둔다
